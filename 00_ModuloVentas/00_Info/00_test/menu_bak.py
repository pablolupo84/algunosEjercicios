...
class ModuloVentas:
	def __init__(self):
		...
		self.botonConsultarCliente = Button(self.miFrame_1, text="CONSULTAR CLIENTE",width=25,command=self.consultarxCliente)
		self.botonConsultarCliente.grid(row=3, column=4,padx=10,pady=10,sticky="we")
		#self.botonConsultarCliente.pack()
		...
		self.botonGenReportes = Button(self.miFrame_3, text="GENERAR REPORTE",width=15,command=self.genReportesApp)
		self.botonGenReportes.grid(row=1, column=3,padx=10,pady=10,sticky="we")
	...
	def realizarVenta(self):
		...
		miTupla=(listadata[0],listadata[1],time.strftime("%d/%m/%y"),time.strftime("%H:%M:%S"),valorVenta*listadata[2],listadata[3])
		#print(miTupla)
		miVentaNueva=Ventas()
		miVentaNueva.NuevaVenta(idArticulo,miTupla)
		articulosTotal=Articulos()
		articulosTotal.ModificarStock(idArticulo,listadata[2])
	...
	def ModificarVenta(self):
		opcion=messagebox.askquestion("Eliminar","Desea eliminar la venta?")
		miVenta=Ventas()
	...
	def consultarxCliente(self):
		...
		self.treeConsultas = ttk.Treeview(self.miFrame,columns = ("id_cliente","id_venta","id_articulo","cantidad","precio_vta"))   
		...
		self.treeConsultas.heading('#3', text='id_articulo', anchor=tk.W)
	...
	def buscarPorCliente(self):
		...
		for i in listdata:
			...
			#print(listdataVentas)
			for a in listdataVentas:
				milista.append(tupladatos + (a[2],a[3],a[4]))
				suma=suma+a[3]*a[4]
				#self.sumatoria.set(self.sumatoria.get()+a[3]*a[4])
		for row in milista:
			self.treeConsultas.insert('',END, values=row)
		#print (self.sumatoria.get())
...
#MODULO GEN REPORTES
	...
	def genReportesApp(self):
		...
		self.botonSet = Button(self.miFrame, text="Generar Reporte", width=15,command=self.genReporte)
